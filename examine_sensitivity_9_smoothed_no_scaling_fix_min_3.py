...
from functions.fabio_add import find_nearest_index,multi_gaussian,all_file_names,load_dark,find_index_of_file,get_metadata,examine_current_trace,movie_from_data,get_bin_and_interv_no_lines, four_point_transform, fix_minimum_signal, do_tilt_no_lines, fix_minimum_signal2, fix_minimum_signal3
...
limits_wave = [66, 84]
figure_number = 0
where_to_save_everything = '/home/ffederic/work/Collaboratory/test/experimental_data/functions/Calibrations/sensitivity_9/Sensitivity_4_plots_'
...
plt.title('Exp '+str(limits_angle)+'\n scaled_up_calBinned_single before Gaussian convolution')
plt.colorbar()
figure_number+=1
plt.savefig(where_to_save_everything+str(figure_number)+'.eps', bbox_inches='tight')
plt.close()
...
np.save('/home/ffederic/work/Collaboratory/test/experimental_data/functions/Calibrations/sensitivity_9/Sensitivity_4',to_save)
...
for iFile, j in enumerate(calib_ID_long):
	...
	for index, filename in enumerate(filenames):
		...
		data = np.array(im)
		data = (data - dataDark)*Gain[index]
		data, fix_done_marker = fix_minimum_signal3(data, fix_done_marker=True)
