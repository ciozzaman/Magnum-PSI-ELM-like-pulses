...
for merge_ID_target in merge_ID_target_multipulse:  # 88 excluded because I don't have a temperature profile
	...
	if H2_suppression:
		...
		externally_provided_H2p_Hn_steps = 11
		externally_provided_H_steps = 13
	...
	for min_nHp_ne,atomic_restricted_high_n in [[0.01,True]]:
		...
		for max_n_neutrals_ne in [1]:
			...
			print('externally_provided_H2p_Hn_steps = '+str(externally_provided_H2p_Hn_steps))
					...
					def calc_stuff_2(arg):
						try:
							...
							if collect_power_PDF:
								to_find_steps = externally_provided_H2p_Hn_steps	# 25/21 no, it was 11
							else:
								to_find_steps = externally_provided_H2p_Hn_steps	# 25/21 no, it was 11
							...
							for loop_index in [0,1]:
								...
								record_nH2p_nH2_values = nH2p_nH2_values_Te_ne(Te_values.flatten(),ne_values.flatten(),to_find_steps,H2_suppression=H2_suppression)
								record_nH2p_ne_values = np.transpose([record_nH2_ne_values.tolist()]*(to_find_steps) ,(1,0,2)) * record_nH2p_nH2_values
								record_nHm_nH2_values = nHm_nH2_values_Te(Te_values.flatten(),to_find_steps,H2_suppression=H2_suppression)
							...
							most_likely_marginalised_nH2p_nH2_index = index_most_likely_marginalised%to_find_steps
							most_likely_marginalised_nHm_nH2_index = (index_most_likely_marginalised-most_likely_marginalised_nH2p_nH2_index)//to_find_steps%to_find_steps
							most_likely_marginalised_nH_ne_index = ((index_most_likely_marginalised-most_likely_marginalised_nH2p_nH2_index)//to_find_steps-most_likely_marginalised_nHm_nH2_index)//to_find_steps%H_steps
							...
							nH_ne_excited_states_atomic = np.transpose(nH_ne_excited_states_atomic, (3,0,1,2,4))
							power_rad_atomic_visible = [[[np.float32(record_nH_ne_values*np.sum(excitation_full*multiplicative_factor_visible_light_full_full,axis=-1)*ne_values.flatten()).tolist()]*(to_find_steps+how_much_expand_nH2p_nH2_indexes)]*H2_steps]*(to_find_steps+how_much_expand_nHm_nH2_indexes)
							...
							# total_nH2_ne_value = np.float32(np.transpose(total_nH2_ne_value, (0,2,1)))	# H2, ne*Te, lines
							total_nH_nH2_ne_values = total_nH_ne_value + total_nH2_ne_value
							record_nH2p_nH2_values = nH2p_nH2_values_Te_ne_expanded(Te_values.flatten(),ne_values.flatten(),to_find_steps,how_expand_nH2p_nH2_indexes,H2_suppression=H2_suppression)
							record_nH2p_nH2_values_initial = nH2p_nH2_values_Te_ne(Te_values.flatten(),ne_values.flatten(),to_find_steps,H2_suppression=H2_suppression)
							...
							record_nHm_nH2_values = nHm_nH2_values_Te_expanded(Te_values.flatten(),to_find_steps,how_expand_nHm_nH2_indexes,H2_suppression=H2_suppression)
							record_nHm_nH2_values_initial = nHm_nH2_values_Te(Te_values.flatten(),to_find_steps,H2_suppression=H2_suppression)
							...
							if True:
								...
								try:
									...
									print('worker '+str(current_process())+' marker 1-1 '+str(domain_index)+' in %.3gmin %.3gsec' %(int(time_lapsed/60),int(time_lapsed%60)) +' for some reason del temp_coord,temp,temp_sigma,temp1,temp1_sigma failed' )
							if include_particles_limitation:
								particles_penalty = np.zeros_like((likelihood_log_probs),dtype=np.float32)
								all_net_Hp_destruction *= area*length*dt/1000
								all_net_e_destruction *= area*length*dt/1000
							...
							if (collect_power_PDF or to_print):
								...
								def build_log_PDF(power,PDF_intervals,use_all=True,treshold_ratio=1.3,require_dict=False):
								...
								intervals_power_rad_Hm_Hp,prob_power_rad_Hm_Hp,actual_values_power_rad_Hm_Hp = build_log_PDF(power_rad_Hm_Hp,PDF_intervals)
								...
								power_balance_data_dict['H_destruction_RR'] = build_log_PDF(all_H_destruction_RR,PDF_intervals,require_dict=True)
								...
								power_balance_data_dict['H2_destruction_RR'] = build_log_PDF(all_H2_destruction_RR,PDF_intervals,require_dict=True)
								...
								power_balance_data_dict['eff_CX_RR'] = build_log_PDF(eff_CX_RR,PDF_intervals,require_dict=True)
								del eff_CX_RR
								# intervals_CX_term_1_1,prob_CX_term_1_1,actual_values_CX_term_1_1 = build_log_PDF(CX_term_1_1,PDF_intervals)
								power_balance_data_dict['CX_term_1_1'] = build_log_PDF(CX_term_1_1,PDF_intervals,require_dict=True)
								del CX_term_1_1
								# intervals_CX_term_1_2,prob_CX_term_1_2,actual_values_CX_term_1_2 = build_log_PDF(CX_term_1_2,PDF_intervals)
								power_balance_data_dict['CX_term_1_2'] = build_log_PDF(CX_term_1_2,PDF_intervals,require_dict=True)
								del CX_term_1_2
								# intervals_CX_term_1_3,prob_CX_term_1_3,actual_values_CX_term_1_3 = build_log_PDF(CX_term_1_3,PDF_intervals)
								power_balance_data_dict['CX_term_1_3'] = build_log_PDF(CX_term_1_3,PDF_intervals,require_dict=True)
								del CX_term_1_3
								# intervals_CX_term_1_4,prob_CX_term_1_4,actual_values_CX_term_1_4 = build_log_PDF(CX_term_1_4,PDF_intervals)
								power_balance_data_dict['CX_term_1_4'] = build_log_PDF(CX_term_1_4,PDF_intervals,require_dict=True)
								del CX_term_1_4
								# intervals_CX_term_1_5,prob_CX_term_1_5,actual_values_CX_term_1_5 = build_log_PDF(CX_term_1_5,PDF_intervals)
								power_balance_data_dict['CX_term_1_5'] = build_log_PDF(CX_term_1_5,PDF_intervals,require_dict=True)
								del CX_term_1_5
								# intervals_CX_term_1_6,prob_CX_term_1_6,actual_values_CX_term_1_6 = build_log_PDF(CX_term_1_6,PDF_intervals)
								power_balance_data_dict['CX_term_1_6'] = build_log_PDF(CX_term_1_6,PDF_intervals,require_dict=True)
								del CX_term_1_6
								# intervals_CX_term_1_7,prob_CX_term_1_7,actual_values_CX_term_1_7 = build_log_PDF(CX_term_1_7,PDF_intervals)
								power_balance_data_dict['CX_term_1_7'] = build_log_PDF(CX_term_1_7,PDF_intervals,require_dict=True)
								del CX_term_1_7
								power_balance_data_dict['CX_term_1_8'] = build_log_PDF(CX_term_1_8,PDF_intervals,require_dict=True)
								del CX_term_1_8
								power_balance_data_dict['CX_term_1_9'] = build_log_PDF(CX_term_1_9,PDF_intervals,require_dict=True)
								del CX_term_1_9
								# power_balance_data_dict['CX_term_1_10'] = build_log_PDF(CX_term_1_10,PDF_intervals,require_dict=True)
								# del CX_term_1_10
								power_balance_data_dict['CX_term_1_11'] = build_log_PDF(CX_term_1_11,PDF_intervals,require_dict=True)
								del CX_term_1_11
								...
								power_balance_data_dict['H_creation_RR'] = build_log_PDF(np.float64(all_H_creation_RR)*1e20,PDF_intervals,require_dict=True)
								del all_H_creation_RR
								# intervals_total_removed_power_visible,prob_total_removed_power_visible,actual_values_total_removed_power_visible = build_log_PDF(total_removed_power_visible,PDF_intervals)
								power_balance_data_dict['total_removed_power_visible'] = build_log_PDF(total_removed_power_visible,PDF_intervals,require_dict=True)
								del total_removed_power_visible
								# intervals_power_rad_atomic_visible,prob_power_rad_atomic_visible,actual_values_power_rad_atomic_visible = build_log_PDF(power_rad_atomic_visible,PDF_intervals)
								power_balance_data_dict['power_rad_atomic_visible'] = build_log_PDF(power_rad_atomic_visible,PDF_intervals,require_dict=True)
								del power_rad_atomic_visible
								# intervals_power_rad_mol_visible,prob_power_rad_mol_visible,actual_values_power_rad_mol_visible = build_log_PDF(power_rad_mol_visible,PDF_intervals)
								power_balance_data_dict['power_rad_mol_visible'] = build_log_PDF(power_rad_mol_visible,PDF_intervals,require_dict=True)
