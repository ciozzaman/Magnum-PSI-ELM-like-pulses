...
if (merge_ID_target<=26 or merge_ID_target==53):
	...
	limits_tilt = [0,0]
	limits_wave = [0,0]
	angle = 0.056376554473999786
	tilt = [2.94069838e+01,   1.78656752e-03,   4.68433224e+01]
	waveLcoefs = np.ones((2, 3)) * np.nan
	waveLcoefs[1] = [ -2.17386695e-06,   1.07318668e-01,   3.40691723e+02]
	...
	limits_tilt = [0,0]
	limits_wave = [0,0]
	angle = 0.181016926656
	tilt = [29.44104251  ,        np.nan,  45.31376518]
	waveLcoefs = np.ones((2, 3)) * np.nan
	waveLcoefs[1] = [-3.29891786e-06  , 1.09635350e-01 ,  3.39719856e+02]
	...
	limits_tilt = [0,0]
	limits_wave = [0,0]
	angle = -0.11492073276666746
	tilt = [2.80757545e+01,   4.54545455e-03,   4.58877282e+01]
	waveLcoefs = np.ones((2, 3)) * np.nan
	waveLcoefs[1] = [-1.19837339e-06,   1.08909615e-01,   3.49589924e+02]  # from examine_sensitivity.py
	...
	if (last_merge_done<=64 and last_merge_done>=58):
		no_calculate_presets = 1
	...
	tilt = [2.41724091e+01 , -1.18060143e-02 ,  6.79100852e+01]
	waveLcoefs = np.ones((2, 3)) * np.nan
	waveLcoefs[1] = [ -1.49222619e-06 ,  1.27428400e-01 ,  3.18066804e+02]
	if (last_merge_done<=100 and last_merge_done>=66):
		no_calculate_presets = 1
...
if ((not time_resolution_scan and not os.path.exists(path_where_to_save_everything + '/merge'+str(merge_ID_target)+'_merge_tot.npz')) or (time_resolution_scan and not os.path.exists(path_where_to_save_everything+'/merge'+str(merge_ID_target)+'_skip_of_'+str(time_resolution_extra_skip+1)+'row_merge_tot.npz'))):
# if True:
...
if ((not time_resolution_scan and not os.path.exists(path_where_to_save_everything+'/merge'+str(merge_ID_target)+'_composed_array.npy')) or (time_resolution_scan and not os.path.exists(path_where_to_save_everything+'/merge'+str(merge_ID_target)+'_skip_of_'+str(time_resolution_extra_skip+1)+'row_composed_array.npy'))):
	...
	# plt.pause(0.001)
	plt.savefig(path_where_to_save_everything+ '/' + 'interpolation_borders_expanded.eps', bbox_inches='tight')
	plt.close()
	...
	if time_resolution_scan:
		...
		ani.save(path_where_to_save_everything+'/merge' + str(merge_ID_target)+'_skip_of_'+str(time_resolution_extra_skip+1)+'row_composed_array' + '.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
...
#
	...
	if time_resolution_scan:
		...
		binned_data = np.load(path_where_to_save_everything+'/merge' + str(merge_ID_target)+'/merge' + str(merge_ID_target) + '_binned_data.npy')
...
doLateralfit_time_tependent(df_settings, all_fits, merge_ID_target,first_time,last_time,conventional_time_step,dx,xx,r)
...
sample_time_step = []
for time in [-2, 0, 0.1,0.15, 0.4, 0.5, 0.7, 1, 2,3, 4,5,6]:	#times in ms that I want to take a better look at
...
for iR in range(np.shape(inverted_profiles)[1] - 1):
	plt.figure(figsize=(20, 10))
	# plt.plot(new_timesteps, inverted_profiles[:, iR, loc] / inverted_profiles[:, iR + 1, loc], color[iR]+'+',label='line ratio ' + str(iR + 4) + '/' + str(iR + 4 + 1))
	plt.imshow(inverted_profiles[:, iR + 1] / inverted_profiles[:, iR], 'rainbow',vmin=0,vmax=1,extent=[min(xx)*1000,max(xx)*1000,np.max(new_timesteps),np.min(new_timesteps)],aspect=20)
...
for iR in range(np.shape(inverted_profiles)[1] - 1):
	plt.figure(figsize=(20, 10))
	# plt.plot(new_timesteps, inverted_profiles[:, iR, loc] / inverted_profiles[:, iR + 1, loc], color[iR]+'+',label='line ratio ' + str(iR + 4) + '/' + str(iR + 4 + 1))
	plt.imshow((inverted_profiles[:, iR + 1] / inverted_profiles[:, iR])*(statistical_weigth[iR] * einstein_coeff[iR] * energy_difference[iR])/(statistical_weigth[iR+1] * einstein_coeff[iR+1] * energy_difference[iR+1]), 'rainbow',vmin=0,extent=[min(xx)*1000,max(xx)*1000,np.max(new_timesteps),np.min(new_timesteps)],aspect=20)
...
all_fits_ss = np.nanmedian(all_fits_ss,axis=0)
