def generate_merge88_TS():
	...
	merge_ID_target_multipulse = [85,86,87,89,95]
	...
	merge_Te_prof_multipulse_interp_crop = np.zeros_like(merge_Te_prof_multipulse_interp_crop)
	merge_dTe_prof_multipulse_interp_crop = np.zeros_like(merge_Te_prof_multipulse_interp_crop)
	merge_ne_prof_multipulse_interp_crop = np.zeros_like(merge_Te_prof_multipulse_interp_crop)
	merge_dne_prof_multipulse_interp_crop = np.zeros_like(merge_Te_prof_multipulse_interp_crop)
	for i_t in range(np.shape(merge_Te_prof_multipulse_interp_crop_all)[1]):
		for i_r in range(np.shape(merge_Te_prof_multipulse_interp_crop_all)[2]):
			bds = [[-np.inf,-np.inf,0],[np.inf,np.inf,np.inf]]
			fit = curve_fit(parabola,target_chamber_pressure_all-target_chamber_pressure,merge_Te_prof_multipulse_interp_crop_all[:,i_t,i_r],p0=[0,1,1],sigma=merge_dTe_prof_multipulse_interp_crop_all[:,i_t,i_r],absolute_sigma=True,bounds = bds)
			temp = parabola(0,*correlated_values(fit[0],fit[1]))
			merge_Te_prof_multipulse_interp_crop[i_t,i_r] = nominal_values(temp)
			merge_dTe_prof_multipulse_interp_crop[i_t,i_r] = std_devs(temp)
			fit = curve_fit(parabola,target_chamber_pressure_all-target_chamber_pressure,merge_ne_prof_multipulse_interp_crop_all[:,i_t,i_r],p0=[0,1,1],sigma=merge_dne_prof_multipulse_interp_crop_all[:,i_t,i_r],absolute_sigma=True,bounds = bds)
			temp = parabola(0,*correlated_values(fit[0],fit[1]))
			merge_ne_prof_multipulse_interp_crop[i_t,i_r] = nominal_values(temp)
			merge_dne_prof_multipulse_interp_crop[i_t,i_r] = std_devs(temp)
			# np.polyval(np.polyfit(target_chamber_pressure_all,merge_ne_prof_multipulse_interp_crop_all[:,i_t,i_r],2),target_chamber_pressure)

	# plt.figure()
	# plt.imshow(merge_dne_prof_multipulse_interp_crop)
	# plt.colorbar()
	# plt.pause(0.01)

	return(merge_Te_prof_multipulse_interp_crop,merge_dTe_prof_multipulse_interp_crop,merge_ne_prof_multipulse_interp_crop,merge_dne_prof_multipulse_interp_crop)
