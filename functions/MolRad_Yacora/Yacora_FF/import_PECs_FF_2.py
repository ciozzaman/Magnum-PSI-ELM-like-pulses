...
def RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	...
	e_H__Hm = merge_ne_prof_multipulse_interp_crop_limited * reaction_rate
	return e_H__Hm

def RR_e_Hm__e_H_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
...
def RR_Hp_Hm__H_2_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
...
def RR_Hp_Hm__H_3_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
...
def RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	...
	if len(np.shape(merge_Te_prof_multipulse_interp_crop_limited))==4:	# H, Hm, H2, H2p, ne, Te
		...
		reaction_rate = (ne.flatten()**2)*1e20*np.sum(population_coefficients*einstein_coeff_Lyman,axis=-1)	# 	#/s * 1e-20
		...
		reaction_rate = (merge_ne_prof_multipulse_interp_crop_limited.flatten()**2)*1e20*(np.sum(population_coefficients*einstein_coeff_Lyman,axis=-1).reshape(np.shape(merge_Te_prof_multipulse_interp_crop_limited)))	# 	#/s * 1e-20
	return reaction_rate

def RR_H2p_Hm__Hex_H2(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nH2p_ne_all):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	flag_single_point=False
...
def RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
...
def RR_Hm_H1s__H1s_H1s_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	Hm_H1s__H1s_H1s_e = merge_ne_prof_multipulse_interp_crop_limited*reaction_rate
	return Hm_H1s__H1s_H1s_e


def RR_Hm_H1s__H2_v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	reaction_rate = list(map(internal,(np.array([T_H.flatten(),T_Hm.flatten()]).T)))
	reaction_rate = np.reshape(reaction_rate,np.shape(T_H))[0]
	Hm_H1s__H2_v_e = merge_ne_prof_multipulse_interp_crop_limited*reaction_rate
	return Hm_H1s__H2_v_e


def RR_Hp_H_H__Hp_H2__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hp,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	Hp_H_H__Hp_H2 = merge_ne_prof_multipulse_interp_crop_limited*reaction_rate
	return Hp_H_H__Hp_H2


def RR_H_H_H__H_H2__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
...
def RR_e_H2_X1Σg_x__e_H_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
		...
		if not np.shape(np.array(merge_Te_prof_multipulse_interp_crop_limited))==():
			...
			if merge_Te_prof_multipulse_interp_crop_limited<0.1:
				reaction_rate = 0
		e_H2_X1Σg_x__e_H_H = reaction_rate * (merge_ne_prof_multipulse_interp_crop_limited**2)
		return e_H2_X1Σg_x__e_H_H

def RR_e_H2__e_Hex_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	flag_single_point=False
	...
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		...
		reaction_rate = (ne**2)*1e20*np.sum(population_coefficients*einstein_coeff_Lyman,axis=-1)	# 	#/s * 1e-20
	else:
...
def RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
		...
		if not np.shape(np.array(merge_Te_prof_multipulse_interp_crop_limited))==():
			...
			if merge_Te_prof_multipulse_interp_crop_limited<0.5:
				reaction_rate = 0
		e_H2N13Λσ__e_H2pX2Σg_e = reaction_rate * (merge_ne_prof_multipulse_interp_crop_limited**2)

	return e_H2N13Λσ__e_H2pX2Σg_e
	# checked up to here


def RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
		...
		if not np.shape(np.array(merge_Te_prof_multipulse_interp_crop_limited))==():
			...
			if merge_Te_prof_multipulse_interp_crop_limited<1:
				reaction_rate = 0
		e_H2N13Λσ__Hp_H_2e = reaction_rate * (merge_ne_prof_multipulse_interp_crop_limited**2)
	return e_H2N13Λσ__Hp_H_2e


def RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return e_H2X1Σg__Hm_H1s


def RR_e_H2X1Σg__e_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return e_H2X1Σg__e_H1s_H1s


def RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	if False:
		...
		reaction_rate[np.logical_not(np.isfinite(reaction_rate))]=0
		Hp_H2X1Σg__H1s_H2pX2Σg = reaction_rate*(merge_ne_prof_multipulse_interp_crop_limited**2)
		reaction_rate[reaction_rate<0]=0
	return Hp_H2X1Σg__H1s_H2pX2Σg

def RR_Hp_H1s__H1s_Hp__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hp,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
...
def RR_Hp_H2X1Σg__Hp_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return Hp_H2X1Σg__Hp_H1s_H1s


def RR_Hm_H2v__H_H2v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	if False:	# NOTE: I'm not using this because the this formulas return something (cros section >0) only for high energies, something I don't have in my cases
		...
		if not np.shape(np.array(merge_Te_prof_multipulse_interp_crop_limited))==():
			reaction_rate[np.logical_not(np.isfinite(reaction_rate))]=0
		Hm_H2v__H_H2v_e = reaction_rate*(merge_ne_prof_multipulse_interp_crop_limited**2)
	return Hm_H2v__H_H2v_e

def RR_H1s_H2v__H1s_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	for v_index,coefficients in enumerate(coefficients_all):
		...
		reaction_rate[np.logical_not(np.isfinite(reaction_rate))]=0
		if v_index==0:
			H1s_H2v__H1s_2H1s[v_index] += reaction_rate *merge_ne_prof_multipulse_interp_crop_limited* fractional_population_states_H2[0]
		else:
			H1s_H2v__H1s_2H1s[v_index] += reaction_rate *merge_ne_prof_multipulse_interp_crop_limited* fractional_population_states_H2[v_index]
	H1s_H2v__H1s_2H1s = np.sum(H1s_H2v__H1s_2H1s,axis=0)
	return H1s_H2v__H1s_2H1s

def RR_H2v0_H2v__H2v0_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return H2v0_H2v__H2v0_2H1s

def RR_e_H2p__XXX__e_Hp_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
		...
		if not np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
			reaction_rate[np.logical_not(np.isfinite(reaction_rate))]=0
		e_H2p__XXX__e_Hp_H1s = (merge_ne_prof_multipulse_interp_crop_limited**2)*reaction_rate
	return e_H2p__XXX__e_Hp_H1s

def RR_e_H2p__H1s_Hn2__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
		...
		if not np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
			reaction_rate[np.logical_not(np.isfinite(reaction_rate))]=0
		e_H2p__H1s_Hn2 = (merge_ne_prof_multipulse_interp_crop_limited**2)*reaction_rate
	return e_H2p__H1s_Hn2

def RR_e_H2p__Hex_H__or__Hex_Hp_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
...
def RR_e_H2p__e_Hp_Hp_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if False:
		if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
			print('error, Te and ne are different shapes')
			exit()
...
def RR_H1s_H2pv__Hp_H_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_H2p,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	H1s_H2pv__Hp_H_H = merge_ne_prof_multipulse_interp_crop_limited*reaction_rate
	return H1s_H2pv__Hp_H_H

def RR_H2pvi_H2v0__Hp_H_H2v1__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2p,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return H2pvi_H2v0__Hp_H_H2v1

def RR_H2pvi_H2v0__H3p_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return H2pvi_H2v0__H3p_H1s


def RR_H2p_Hm__H2N13Λσ_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return H2p_Hm__H2N13Λσ_H1s


def RR_H2p_Hm__H3p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	return H2p_Hm__H3p_e


def RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	reaction_rate_max = reaction_rate.flatten()[(np.abs(temperature-30000)).argmin()]
	reaction_rate[temperature>30000]=reaction_rate_max
	Hp_H_H__H_H2p = merge_ne_prof_multipulse_interp_crop_limited*reaction_rate
	return Hp_H_H__H_H2p


def RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	def internal(T_H):
		...
		cross_section = gauss(baricenter_impact_energy,2.5 * (1e-17 * 1e-4 * 1e20),0.9,3.25)
	...
	reaction_rate = np.reshape(reaction_rate,np.shape(T_H))[0]
	H1s_H_2__H2p_e = reaction_rate
	return H1s_H_2__H2p_e

def RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	if False:	# Probrem: the approximation of the true cross section with this function is way too inaccurate
		...
		cross_section[np.logical_and(baricenter_impact_energy<=0.1,baricenter_impact_energy>0)] += 2.96*(3**4)*1e-19*1e-4/(baricenter_impact_energy[np.logical_and(baricenter_impact_energy<=0.1,baricenter_impact_energy>0)]) * 1e20
		cross_section[np.logical_and(baricenter_impact_energy<=1,baricenter_impact_energy>0.1)] += 2.96*(3**4)*1e-18*1e-4 * 1e20
		cross_section[baricenter_impact_energy>1] += 2.96*(3**4)*1e-18*1e-4/(baricenter_impact_energy[baricenter_impact_energy>1]**(0.4*3)) * 1e20
		cross_section[cross_section<0] = 0
		...
		reaction_rate = list(map(internal,(np.array([T_H.flatten()]).T)))
		reaction_rate = np.reshape(reaction_rate,np.shape(T_H))[0]
		H1s_H_3__H2p_e = reaction_rate
	return H1s_H_3__H2p_e

def RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)==():
	...
	def internal(T_H):
		...
		cross_section[baricenter_impact_energy>1] += 2.96*(4**4)*1e-18*1e-4/(baricenter_impact_energy[baricenter_impact_energy>1]**(0.4*4)) * 1e20
		cross_section[cross_section<0] = 0
		reaction_rate = np.nansum((cross_section*H_1_velocity*H_1_velocity_PDF).T * H_2_velocity*H_2_velocity_PDF ,axis=(0,-1)).T* np.mean(np.diff(H_1_velocity))*np.mean(np.diff(H_2_velocity))		# m^3 / s
		# reaction_rate = np.sum((cross_section*H_velocity*H_velocity_PDF).T * H_velocity*H_velocity_PDF )* np.mean(np.diff(H_velocity))		# m^3 / s
		return reaction_rate
	reaction_rate = list(map(internal,(np.array([T_H.flatten()]).T)))
	reaction_rate = np.reshape(reaction_rate,np.shape(T_H))[0]
	H1s_H_4__H2p_e = reaction_rate
	return H1s_H_4__H2p_e



def RR_ionisation__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	flag_single_point=False
	...
	if flag_single_point:
		return effective_ionisation_rates[0]
	else:
		return effective_ionisation_rates

def RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited):
	if np.shape(merge_Te_prof_multipulse_interp_crop_limited)!=np.shape(merge_ne_prof_multipulse_interp_crop_limited):
		print('error, Te and ne are different shapes')
		exit()
	flag_single_point=False
	...
	if flag_single_point:
		return effective_recombination_rates[0]
	else:
		return effective_recombination_rates


def RR_rate_creation_Hm(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		out = nH_all_ground_state*RR_e_H__Hm__r(Te,ne)[0] + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0]
		out_sigma = ((molecular_precision*nH_all_ground_state*RR_e_H__Hm__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0])**2)**0.5
	else:
		out = nH_all_ground_state*RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2)
		out_sigma = ((molecular_precision*nH_all_ground_state*RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2)**0.5
	return out,out_sigma

def RR_rate_destruction_Hm(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
...
def RR_rate_creation_H2p(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		out = nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0] + population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(Te,np.unique(T_H),ne) + population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(Te,np.unique(T_H),ne) + population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(Te,np.unique(T_H),ne)
		out_sigma = ((molecular_precision*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0])**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0])**2 + (molecular_precision*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0])**2 + (molecular_precision*population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(Te,np.unique(T_H),ne))**2)**0.5
	else:
		out = nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited)
		out_sigma = ((molecular_precision*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2)**0.5
	return out,out_sigma
...
def RR_rate_destruction_H2(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		# return nH2_ne_all*RR_e_H2_X1Σg_x__e_H_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0] + nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(Te,ne,fractional_population_states_H2)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(Te,np.unique(T_Hp),ne,fractional_population_states_H2)[0] + nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(Te,np.unique(T_H),ne,fractional_population_states_H2)[0] + (nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(Te,np.unique(T_H2),ne,fractional_population_states_H2)[0] + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(Te,np.unique(T_H2),np.unique(T_H2p),ne,fractional_population_states_H2)[0]
		out = nH2_ne_all*RR_e_H2__e_Hex_H__r(Te,ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0] + nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(Te,ne,fractional_population_states_H2)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(Te,np.unique(T_Hp),ne,fractional_population_states_H2)[0] + nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(Te,np.unique(T_H),ne,fractional_population_states_H2)[0] + (nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(Te,np.unique(T_H2),ne,fractional_population_states_H2)[0] + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(Te,np.unique(T_H2),np.unique(T_H2p),ne,fractional_population_states_H2)[0]
		out_sigma = ((molecular_precision*nH2_ne_all*RR_e_H2__e_Hex_H__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(Te,ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0])**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(Te,np.unique(T_Hp),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(Te,np.unique(T_H),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*(nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(Te,np.unique(T_H2),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(Te,np.unique(T_H2),np.unique(T_H2p),ne,fractional_population_states_H2)[0])**2)**0.5
	else:
		# return nH2_ne_all*RR_e_H2_X1Σg_x__e_H_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + (nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2)
		out = nH2_ne_all*RR_e_H2__e_Hex_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + (nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2)
		out_sigma = ((molecular_precision*nH2_ne_all*RR_e_H2__e_Hex_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2 + (molecular_precision*nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2 + (molecular_precision*(nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2 + (molecular_precision*nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2)**0.5
	return out,out_sigma
...
def RR_rate_creation_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		# return nHm_ne_all*RR_e_Hm__e_H_e__r(Te,ne)[0] + 2*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_2_H__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + 2*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_3_H__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0] + 2*nH2_ne_all*RR_e_H2_X1Σg_x__e_H_H__r(Te,ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0] + 2*nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(Te,ne,fractional_population_states_H2)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(Te,np.unique(T_Hp),ne,fractional_population_states_H2)[0] + nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(Te,np.unique(T_Hm),ne)[0] + 2*nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(Te,np.unique(T_H),ne,fractional_population_states_H2)[0] + 2*(nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(Te,np.unique(T_H2),ne,fractional_population_states_H2)[0] + nH2p_ne_all*RR_e_H2p__XXX__e_Hp_H1s__r(Te,ne)[0] + 2*nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(Te,ne)[0] + nH_all_ground_state*nH2p_ne_all*RR_H1s_H2pv__Hp_H_H__r(Te,np.unique(T_H),np.unique(T_H2p),ne)[0] + nH2_ne_all*nH2p_ne_all*RR_H2pvi_H2v0__Hp_H_H2v1__r(Te,np.unique(T_H2p),ne)[0] + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(Te,np.unique(T_H2),np.unique(T_H2p),ne,fractional_population_states_H2)[0] + nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H2N13Λσ_H1s__r(Te,np.unique(T_Hm),np.unique(T_H2p),ne)[0] + nHm_ne_all*RR_H2p_Hm__Hex_H2(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nH2p_ne_all) + nHp_ne_all*RR_recombination__r(Te,ne)[0]
		out = nHm_ne_all*RR_e_Hm__e_H_e__r(Te,ne)[0] + 2*nHm_ne_all*RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all) + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0] + 2*nH2_ne_all*RR_e_H2__e_Hex_H__r(Te,ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0] + 2*nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(Te,ne,fractional_population_states_H2)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(Te,np.unique(T_Hp),ne,fractional_population_states_H2)[0] + nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(Te,np.unique(T_Hm),ne)[0] + 2*nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(Te,np.unique(T_H),ne,fractional_population_states_H2)[0] + 2*(nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(Te,np.unique(T_H2),ne,fractional_population_states_H2)[0] + nH2p_ne_all*RR_e_H2p__XXX__e_Hp_H1s__r(Te,ne)[0] + 2*nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(Te,ne)[0] + nH_all_ground_state*nH2p_ne_all*RR_H1s_H2pv__Hp_H_H__r(Te,np.unique(T_H),np.unique(T_H2p),ne)[0] + nH2_ne_all*nH2p_ne_all*RR_H2pvi_H2v0__Hp_H_H2v1__r(Te,np.unique(T_H2p),ne)[0] + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(Te,np.unique(T_H2),np.unique(T_H2p),ne,fractional_population_states_H2)[0] + nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H2N13Λσ_H1s__r(Te,np.unique(T_Hm),np.unique(T_H2p),ne)[0] + nHm_ne_all*RR_H2p_Hm__Hex_H2(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nH2p_ne_all) + nHp_ne_all*RR_recombination__r(Te,ne)[0]
		out_sigma = ((molecular_precision*nHm_ne_all*RR_e_Hm__e_H_e__r(Te,ne)[0])**2 + (molecular_precision*2*nHm_ne_all*RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all))**2 + (molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*2*nH2_ne_all*RR_e_H2__e_Hex_H__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0])**2 + (molecular_precision*2*nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(Te,ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0])**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(Te,np.unique(T_Hp),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(Te,np.unique(T_Hm),ne)[0])**2 + (molecular_precision*2*nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(Te,np.unique(T_H),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*2*(nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(Te,np.unique(T_H2),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nH2p_ne_all*RR_e_H2p__XXX__e_Hp_H1s__r(Te,ne)[0])**2 + (molecular_precision*2*nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(Te,ne)[0])**2 + (molecular_precision*nH_all_ground_state*nH2p_ne_all*RR_H1s_H2pv__Hp_H_H__r(Te,np.unique(T_H),np.unique(T_H2p),ne)[0])**2 + (molecular_precision*nH2_ne_all*nH2p_ne_all*RR_H2pvi_H2v0__Hp_H_H2v1__r(Te,np.unique(T_H2p),ne)[0])**2 + (molecular_precision*nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(Te,np.unique(T_H2),np.unique(T_H2p),ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H2N13Λσ_H1s__r(Te,np.unique(T_Hm),np.unique(T_H2p),ne)[0])**2 + (molecular_precision*nHm_ne_all*RR_H2p_Hm__Hex_H2(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nH2p_ne_all))**2 + (atomic_precision*nHp_ne_all*RR_recombination__r(Te,ne)[0])**2)**0.5
	else:
		# return nHm_ne_all*RR_e_Hm__e_H_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + 2*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_2_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + 2*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_3_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + 2*nH2_ne_all*RR_e_H2_X1Σg_x__e_H_H__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + 2*nH2_ne_all*RR_e_H2X1Σg__e_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__Hp_H1s_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + 2*nH_all_ground_state*nH2_ne_all*RR_H1s_H2v__H1s_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + 2*(nH2_ne_all**2)*RR_H2v0_H2v__H2v0_2H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2p_ne_all*RR_e_H2p__XXX__e_Hp_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + 2*nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH_all_ground_state*nH2p_ne_all*RR_H1s_H2pv__Hp_H_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_H2p,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*nH2p_ne_all*RR_H2pvi_H2v0__Hp_H_H2v1__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2p,merge_ne_prof_multipulse_interp_crop_limited) + nH2p_ne_all*nH2_ne_all*RR_H2pvi_H2v0__H3p_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H2N13Λσ_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*RR_H2p_Hm__Hex_H2(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nH2p_ne_all) + nHp_ne_all*RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited)
...
def RR_rate_destruction_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		out = nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0] + 2*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__Hp_H2__r(Te,np.unique(T_H),np.unique(T_Hp),ne)[0] + 2*(nH_ne_all**3)*RR_H_H_H__H_H2__r(Te,np.unique(T_H),ne)[0] + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0] + population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(Te,np.unique(T_H),ne) + population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(Te,np.unique(T_H),ne) + population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(Te,np.unique(T_H),ne) + nH_all_ground_state*RR_e_H__Hm__r(Te,ne)[0] + nH_ne_all*RR_ionisation__r(Te,ne)[0]
		out_sigma = ((molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*2*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__Hp_H2__r(Te,np.unique(T_H),np.unique(T_Hp),ne)[0])**2 + (molecular_precision*2*(nH_ne_all**3)*RR_H_H_H__H_H2__r(Te,np.unique(T_H),ne)[0])**2 + (molecular_precision*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0])**2 + (molecular_precision*population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*nH_all_ground_state*RR_e_H__Hm__r(Te,ne)[0])**2 + (atomic_precision*nH_ne_all*RR_ionisation__r(Te,ne)[0])**2)**0.5
	else:
		out = nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + 2*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__Hp_H2__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + 2*(nH_ne_all**3)*RR_H_H_H__H_H2__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + nH_all_ground_state*RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH_ne_all*RR_ionisation__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited)
		out_sigma = ((molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*2*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__Hp_H2__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hp,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*2*(nH_ne_all**3)*RR_H_H_H__H_H2__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH_all_ground_state*RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (atomic_precision*nH_ne_all*RR_ionisation__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2)**0.5
	return out,out_sigma

def RR_rate_creation_Hp(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		out = nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0] + nH2p_ne_all*RR_e_H2p__XXX__e_Hp_H1s__r(Te,ne)[0] + 2*nH2p_ne_all*RR_e_H2p__e_Hp_Hp_e__r(Te,ne)[0] + nH_all_ground_state*nH2p_ne_all*RR_H1s_H2pv__Hp_H_H__r(Te,np.unique(T_H),np.unique(T_H2p),ne) + nH2_ne_all*nH2p_ne_all*RR_H2pvi_H2v0__Hp_H_H2v1__r(Te,np.unique(T_H2p),ne)[0] + nH_ne_all*RR_ionisation__r(Te,ne)[0]
		out_sigma = ((molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0])**2 + (molecular_precision*nH2p_ne_all*RR_e_H2p__XXX__e_Hp_H1s__r(Te,ne)[0])**2 + (molecular_precision*2*nH2p_ne_all*RR_e_H2p__e_Hp_Hp_e__r(Te,ne)[0])**2 + (molecular_precision*nH_all_ground_state*nH2p_ne_all*RR_H1s_H2pv__Hp_H_H__r(Te,np.unique(T_H),np.unique(T_H2p),ne))**2 + (molecular_precision*nH2_ne_all*nH2p_ne_all*RR_H2pvi_H2v0__Hp_H_H2v1__r(Te,np.unique(T_H2p),ne)[0])**2 + (atomic_precision*nH_ne_all*RR_ionisation__r(Te,ne)[0])**2)**0.5
	else:
...
def RR_rate_destruction_Hp(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		# return nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_2_H__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_3_H__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0] + nHp_ne_all*RR_recombination__r(Te,ne)[0]
		out = nHm_ne_all*RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all) + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0] + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0] + nHp_ne_all*RR_recombination__r(Te,ne)[0]
		out_sigma = ((molecular_precision*nHm_ne_all*RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all))**2 + (molecular_precision*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(Te,np.unique(T_H2),np.unique(T_Hp),ne)[0])**2 + (molecular_precision*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(Te,np.unique(T_Hp),np.unique(T_H),ne)[0])**2 + (atomic_precision*nHp_ne_all*RR_recombination__r(Te,ne)[0])**2)**0.5
	else:
		# return nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_2_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H_3_H__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited)
		out = nHm_ne_all*RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all) + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited)
		out_sigma = ((molecular_precision*nHm_ne_all*RR_Hp_Hm__Hex_H(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all))**2 + (molecular_precision*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHp_ne_all*nH2_ne_all*RR_Hp_H2X1Σg__H1s_H2pX2Σg__r(merge_Te_prof_multipulse_interp_crop_limited,T_H2,T_Hp,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHp_ne_all*(nH_all_ground_state**2)*RR_Hp_H_H__H_H2p__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (atomic_precision*nHp_ne_all*RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2)**0.5
	return out,out_sigma

def RR_rate_creation_e(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		out = nHm_ne_all*RR_e_Hm__e_H_e__r(Te,ne)[0] + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0] + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0] + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0] + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0] + nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(Te,np.unique(T_Hm),ne)[0] + nH2p_ne_all*RR_e_H2p__e_Hp_Hp_e__r(Te,ne)[0] + nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H3p_e__r(Te,np.unique(T_Hm),np.unique(T_H2p),ne)[0] + population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(Te,np.unique(T_H),ne) + population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(Te,np.unique(T_H),ne) + population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(Te,np.unique(T_H),ne) + nH_ne_all*RR_ionisation__r(Te,ne)[0]
		out_sigma = ((molecular_precision*nHm_ne_all*RR_e_Hm__e_H_e__r(Te,ne)[0])**2 + (molecular_precision*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(Te,np.unique(T_Hp),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(Te,np.unique(T_H),np.unique(T_Hm),ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(Te,ne)[0])**2 + (molecular_precision*nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(Te,np.unique(T_Hm),ne)[0])**2 + (molecular_precision*nH2p_ne_all*RR_e_H2p__e_Hp_Hp_e__r(Te,ne)[0])**2 + (molecular_precision*nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H3p_e__r(Te,np.unique(T_Hm),np.unique(T_H2p),ne)[0])**2 + (molecular_precision*population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(Te,np.unique(T_H),ne))**2 + (molecular_precision*population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(Te,np.unique(T_H),ne))**2 + (atomic_precision*nH_ne_all*RR_ionisation__r(Te,ne)[0])**2)**0.5
	else:
		out = nHm_ne_all*RR_e_Hm__e_H_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,merge_ne_prof_multipulse_interp_crop_limited) + nH2p_ne_all*RR_e_H2p__e_Hp_Hp_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H3p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited) + population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited) + nH_ne_all*RR_ionisation__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited)
		out_sigma = ((molecular_precision*nHm_ne_all*RR_e_Hm__e_H_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHm_ne_all*nHp_ne_all*RR_Hp_Hm__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H1s_H1s_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHm_ne_all*nH_all_ground_state*RR_Hm_H1s__H2_v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__e_H2pX2Σg_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2N13Λσ__Hp_H_2e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nHm_ne_all*nH2_ne_all*RR_Hm_H2v__H_H2v_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2p_ne_all*RR_e_H2p__e_Hp_Hp_e__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2p_ne_all*nHm_ne_all*RR_H2p_Hm__H3p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[0]*nH_all_ground_state*RR_H1s_H_2__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[1]*nH_all_ground_state*RR_H1s_H_3__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*population_states[2]*nH_all_ground_state*RR_H1s_H_4__H2p_e__r(merge_Te_prof_multipulse_interp_crop_limited,T_H,merge_ne_prof_multipulse_interp_crop_limited))**2 + (atomic_precision*nH_ne_all*RR_ionisation__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2)**0.5
	return out,out_sigma
...
def RR_rate_destruction_e(merge_Te_prof_multipulse_interp_crop_limited,T_Hp,T_H,T_H2,T_Hm,T_H2p,merge_ne_prof_multipulse_interp_crop_limited,nHp_ne_all,nH_ne_all,nH2_ne_all,nHm_ne_all,nH2p_ne_all,fractional_population_states_H2,nH_all_ground_state,population_states,molecular_precision,atomic_precision):
	if (len(np.unique(merge_Te_prof_multipulse_interp_crop_limited))==1 and len(np.unique(merge_ne_prof_multipulse_interp_crop_limited))==1):
		Te = np.unique(merge_Te_prof_multipulse_interp_crop_limited)
		ne = np.unique(merge_ne_prof_multipulse_interp_crop_limited)
		out = nH_all_ground_state*RR_e_H__Hm__r(Te,ne)[0] + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0] + nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(Te,ne) + nHp_ne_all*RR_recombination__r(Te,ne)[0]
		out_sigma = ((molecular_precision*nH_all_ground_state*RR_e_H__Hm__r(Te,ne)[0])**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(Te,ne,fractional_population_states_H2)[0])**2 + (molecular_precision*nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(Te,ne))**2 + (atomic_precision*nHp_ne_all*RR_recombination__r(Te,ne)[0])**2)**0.5
	else:
		out = nH_all_ground_state*RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2) + nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited) + nHp_ne_all*RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited)
		out_sigma = ((molecular_precision*nH_all_ground_state*RR_e_H__Hm__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (molecular_precision*nH2_ne_all*RR_e_H2X1Σg__Hm_H1s__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited,fractional_population_states_H2))**2 + (molecular_precision*nH2p_ne_all*RR_e_H2p__H1s_Hn2__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2 + (atomic_precision*nHp_ne_all*RR_recombination__r(merge_Te_prof_multipulse_interp_crop_limited,merge_ne_prof_multipulse_interp_crop_limited))**2)**0.5
	return out,out_sigma
...
# # from AMJUEL
# # Reaction 2.0c e +H2(v) →H2+ + ... , Ratio H2+ /H2
# coefficients_all = [[-5.179118614571e+00,1.286086917362e-02,-1.247224025136e-02,4.533679343348e-03,-8.361635510932e-04,8.553044625491e-05,-4.999715093023e-06,1.571098567732e-07,-2.060813085997e-09],
# [2.724390078109e+00,2.834163745797e-02,-3.864050364482e-02,1.706652099436e-02,-3.663038423578e-03,4.202564138138e-04,-2.662584829521e-05,8.795062121815e-07,-1.182222955604e-08],
# [-4.386686018740e+00,-2.926061072808e-02,2.894878695435e-02,-9.446648200804e-03,1.518996106179e-03,-1.265997327758e-04,5.854399926391e-06,-1.486122653306e-07,1.689012430939e-09],
# [2.264569877939e+00,1.815438259403e-02,-1.085596419844e-02,3.413100865417e-03,-5.414461499166e-04,5.583904028460e-05,-3.494312204501e-06,1.176151828469e-07,-1.631239647294e-09],
# [7.274238145138e-02,-8.711469240181e-03,1.228482582919e-03,-2.783712092681e-04,4.118527168273e-05,-1.416596336268e-05,1.434270108351e-06,-5.449461807998e-08,6.856555143001e-10],
# [-3.332379782334e-01,3.737790746246e-03,-2.689540529741e-04,-7.626065770945e-05,3.568228589947e-05,-1.406315371249e-06,-3.197342970575e-08,-4.539301068809e-10,9.975543797298e-11],
# [9.526327139861e-02,-1.046333532366e-03,2.549358736065e-04,-3.059560467433e-05,-3.460893323989e-06,4.064186524949e-07,-3.342848157445e-08,2.469622408760e-09,-6.675572336239e-11],
# [-1.096455316607e-02,1.457992401832e-04,-5.912048929304e-05,1.303239138604e-05,-1.086383020673e-06,6.161003040376e-08,2.239916154599e-10,-2.455363614572e-10,7.954115052237e-12],
# [4.636081955869e-04,-7.621573981592e-06,3.973565261183e-06,-1.046764002685e-06,1.291713097220e-07,-9.932517910526e-09,3.412487324650e-10,1.748563436447e-12,-2.514652180013e-13]]
# e_H2v__H2p_any___n_H2p_n_H2 = np.exp(np.polynomial.polynomial.polyval2d(np.log(merge_ne_prof_multipulse_interp_crop_limited*1e6),np.log(merge_Te_prof_multipulse_interp_crop_limited),coefficients_all)) * 1e-6	# m^2
# reaction_rate[np.logical_not(np.isfinite(reaction_rate))]=0
# e_H2_X1Σg_x__e_H_H = reaction_rate * merge_ne_prof_multipulse_interp_crop_limited*1e20 * merge_ne_prof_multipulse_interp_crop_limited*nH2_ne_all
...
def nH2_ne_values_Te_2(Te,H2_steps):
	...
	max_nH2_ne = nH2_ne_fit_from_simulations(Te)*40
	min_nH2_ne = nH2_ne_fit_from_simulations(Te)/10
...
def nH2p_nH2_values_Te_ne(Te,ne,to_find_steps,H2_suppression=False):
	...
	if np.shape(Te)==():
		...
		nH2p_nH2_values = np.logspace(np.log10(additional_low_multiplier*max(1e-9,1e-2*min(H2p_H2_v_ratio_AMJUEL(ne,Te),H2p_H2_v0_ratio_AMJUEL(ne,Te)))),np.log10(1/additional_low_multiplier*1),num=to_find_steps)
	else:
		temp=[]
		for i in range(len(Te)):
			...
			temp.append(np.logspace(np.log10(additional_low_multiplier*max(1e-9,1e-2*min(H2p_H2_v_ratio_AMJUEL(ne[i],Te[i]),H2p_H2_v0_ratio_AMJUEL(ne[i],Te[i])))),np.log10(1/additional_low_multiplier*1),num=to_find_steps))
		nH2p_nH2_values = np.array(temp).T
	return nH2p_nH2_values
def nH2p_nH2_values_Te_ne_expanded(Te,ne,to_find_steps,how_expand_nH2p_nH2_indexes,H2_suppression=False):
	...
	if np.shape(Te)==():
		...
		for i in range(len(Te)):
			...
			nH2p_nH2_values = np.logspace(np.log10(additional_low_multiplier*max(1e-9,1e-2*min(H2p_H2_v_ratio_AMJUEL(ne[i],Te[i]),H2p_H2_v0_ratio_AMJUEL(ne[i],Te[i])))),np.log10(1/additional_low_multiplier*1),num=to_find_steps)
			nH2p_nH2_additional_values=[]
			for i in range(to_find_steps-1):
				if how_expand_nH2p_nH2_indexes[i]!=0:
					nH2p_nH2_additional_values.extend(np.logspace(np.log10(nH2p_nH2_values[i]),np.log10(nH2p_nH2_values[i+1]),num=how_expand_nH2p_nH2_indexes[i]+2)[1:-1])
...
def nHm_nH2_values_Te(Te,to_find_steps,H2_suppression=False):
	...
	if H2_suppression:
		additional_low_multiplier = 0.1
	if np.shape(Te)==():
		nHm_nH2_values = np.logspace(np.log10(additional_low_multiplier*max(1e-10,1e-2*min(Hm_H2_v_ratio_AMJUEL(Te),Hm_H2_v0_ratio_AMJUEL(Te)))),np.log10(1/additional_low_multiplier*1),num=to_find_steps)
	else:
		temp=[]
		for i in range(len(Te)):
			temp.append(np.logspace(np.log10(additional_low_multiplier*max(1e-10,1e-2*min(Hm_H2_v_ratio_AMJUEL(Te[i]),Hm_H2_v0_ratio_AMJUEL(Te[i])))),np.log10(1/additional_low_multiplier*1),num=to_find_steps))
		nHm_nH2_values = np.array(temp).T
	return nHm_nH2_values
def nHm_nH2_values_Te_expanded(Te,to_find_steps,how_expand_nHm_nH2_indexes,H2_suppression=False):
	...
	if np.shape(Te)==():
		...
		temp=[]
		for i in range(len(Te)):
			nHm_nH2_values = np.logspace(np.log10(additional_low_multiplier*max(1e-10,1e-2*min(Hm_H2_v_ratio_AMJUEL(Te[i]),Hm_H2_v0_ratio_AMJUEL(Te[i])))),np.log10(1/additional_low_multiplier*1),num=to_find_steps)
			nHm_nH2_additional_values=[]
			for i in range(to_find_steps-1):
				if how_expand_nHm_nH2_indexes[i]!=0:
					nHm_nH2_additional_values.extend(np.logspace(np.log10(nHm_nH2_values[i]),np.log10(nHm_nH2_values[i+1]),num=how_expand_nHm_nH2_indexes[i]+2)[1:-1])
