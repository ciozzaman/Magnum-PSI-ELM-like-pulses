...
for j in all_j:
	...
	data_mean = np.mean(data_all, axis=0)
	try:
		angle=[(get_angle_2(data_mean, nLines=5))]
		print(angle)
	except:
		print('FAILED')
angle = np.array(angle)
...
angle = np.nansum(angle[:, 0] / (angle[:, 1] ** 2)) / np.nansum(1 / angle[:, 1] ** 2)
data_mean = rotate(data_mean, angle)
tilt_4_points=get_4_points(data_mean, nLines=5)
data_mean = four_point_transform(data_mean, tilt_4_points)
tilt=get_tilt(data_mean, nLines=5)
geom = pd.DataFrame(columns = ['angle','tilt','binInterv','bin00a','bin00b'])
geom.loc[0] = [angle, tilt_4_points, tilt[0], tilt[2], tilt[2]]
# waveLcoefs = do_waveL_Calib(100, df_settings, df_log, geom, fdir=fdir)
waveLcoefs = do_waveL_Calib(56, df_settings, df_log, geom, fdir=fdir)

waveaxis = np.polyval(waveLcoefs[1],np.arange(np.shape(data_mean)[1]))
temp_w,temp_LOS = np.meshgrid(waveaxis,np.arange(np.shape(data_mean)[0]))
plt.figure(figsize=(20, 10))
plt.pcolor(temp_w,temp_LOS,data_mean,cmap='rainbow',vmax=np.max(data_mean[:,:800]))
plt.colorbar()
for i in range(len(waveLengths)):
	plt.plot([waveLengths[i],waveLengths[i]],[0,np.shape(data_mean)[0]-1],'--k',linewidth=0.5)
for i in range(41):
...
# plt.figure(figsize=(20, 10))
# plt.imshow(data,'rainbow',origin='lower',vmax=np.max(data[:,:800]))
# plt.colorbar()
# plt.pause(0.01)
# plt.figure(figsize=(20, 10))
# plt.plot(np.sum(data[:,400:1200],axis=0))
# plt.pause(0.01)
...
for iFile, j in enumerate(calib_ID):
	...
	data = (np.array(data).T + additive_factor).T
	data_sigma = np.sqrt((data_sigma**2).T + (additive_factor_sigma**2)).T
	temp1_sigma = np.sqrt((data_sigma*Gain[index])**2 + Noise[index]**2)
	data_sigma_up = data+data_sigma
	data_sigma_down = data-data_sigma
	data_sigma_up = apply_proportionality_calibration(data_sigma_up,x_calibration,y_calibration)
	data_sigma_down = apply_proportionality_calibration(data_sigma_down,x_calibration,y_calibration)
	...
	data1 = (data-dataDark_non_proportional)*Gain[index]
	data = apply_proportionality_calibration(data,x_calibration,y_calibration)
	...
	data = data/(df_log.loc[j, ['Calib_ID']][0]*1000000)
	data_all_for_sigma = data/(df_log.loc[j, ['Calib_ID']][0]*1000000)
	data1 = data1/(df_log.loc[j, ['Calib_ID']][0]*1000000)
...
data = four_point_transform(data, tilt_4_points)
data1 = four_point_transform(data1, tilt_4_points)

waveaxis = np.polyval(waveLcoefs[1],np.arange(np.shape(data_mean)[1]))
temp_w,temp_LOS = np.meshgrid(waveaxis,np.arange(np.shape(data_mean)[0]))
plt.figure(figsize=(20, 10))
plt.pcolor(temp_w,temp_LOS,data,cmap='rainbow')
plt.colorbar()
for i in range(len(waveLengths)):
	plt.plot([waveLengths[i],waveLengths[i]],[0,np.shape(data)[0]-1],'--k',linewidth=0.5)
for i in range(41):
	plt.plot([np.min(waveaxis),np.max(waveaxis)],[geom['bin00b']+i*geom['binInterv'],geom['bin00b']+i*geom['binInterv']],'--k',linewidth=0.5)
plt.title('example of the binning that will be done \n'+str(geom))
figure_number+=1
...
plt.close()
plt.figure(figsize=(20, 10))
plt.plot(np.sum(data[:,100:900],axis=0))
plt.pause(0.01)
...
data_sigma = four_point_transform(data_sigma, tilt_4_points)
...
for i in range(41):
	plt.plot([0,np.shape(data)[-1]],[first_bin+i*bin_interv,first_bin+i*bin_interv],'--k',linestyle=(0, (5, 4, 1, 4)),linewidth=0.5)
...
for i in range(41):
	plt.plot([0,np.shape(data1)[-1]],[first_bin+i*bin_interv,first_bin+i*bin_interv],'--k',linestyle=(0, (5, 4, 1, 4)),linewidth=0.5)
...
for kernel in [[1,3],[1,5],[1,7]]:	#added 12/02/2020
	...
	if calculate_sigma_from_std == True:
		data_sigma = np.std(data_all_for_sigma,axis=0)
	else:
		data_sigma = np.mean(data_sigma_all,axis=(0))
...
binned_data_no_mask,binned_data_no_mask_sigma = binData_with_sigma(data,data_sigma, first_bin, bin_interv)
# binned_data_no_mask = binned_data_no_mask - min(np.min(binned_data_no_mask),0)
longCal_no_mask,longCal_no_mask_sigma = binData_with_sigma(data,data_sigma,first_bin-bin_interv,bin_interv,nCh = 42)
...
plt.plot(np.mean(calBinned_no_mask,axis=0),label='0.02')
...
plt.plot(np.mean(calBinned_no_mask_smoothed,axis=0),label='after')
plt.plot(np.mean(scaled_up_calBinned_single,axis=0),label='scaled up')
...
np.save('/home/ffederic/work/Collaboratory/test/experimental_data/functions/Calibrations/sensitivity_11/Sensitivity_4_non_convoluted',to_save)
...
np.save('/home/ffederic/work/Collaboratory/test/experimental_data/functions/Calibrations/sensitivity_11/Sensitivity_4_non_convoluted_sigma',to_save)
