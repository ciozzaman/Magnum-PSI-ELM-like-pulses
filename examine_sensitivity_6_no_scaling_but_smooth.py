...
# calBinned_no_mask = calBinned_no_mask + max(-np.min(calBinned_no_mask),0)		# I don't do this because it would separate the average from the actual sensitivity too much
...
calibration = calibration + max(-np.min(calibration),0)+0.01	# 0.01 it's arbitrary just to avoid 1/0
...
np.save('/home/ffederic/work/Collaboratory/test/experimental_data/functions/Calibrations/Sensitivity_4',to_save)
...
geom = pd.DataFrame(columns = ['angle','tilt','binInterv','bin00a','bin00b'])
geom.loc[0] = [ -0.11492073276666746, 4.54545455e-03, 2.80757545e+01, 4.58877282e+01, 4.58877282e+01]
waveLcoefs = np.ones((2, 3)) * np.nan
waveLcoefs[1] = [ -1.19837339e-06,   1.08909615e-01,   3.49589924e+02]
geom_null = pd.DataFrame(columns=['angle', 'tilt', 'binInterv', 'bin00a', 'bin00b'])
geom_null.loc[0] = [0, 0, 0, 0, 0]
...
geom = pd.DataFrame(columns = ['angle','tilt','binInterv','bin00a','bin00b'])
geom.loc[0] = [ 0.056376554473999786, 0.00178656752, 29.4069838, 46.8433224, 46.8433224]
waveLcoefs = np.ones((2, 3)) * np.nan
