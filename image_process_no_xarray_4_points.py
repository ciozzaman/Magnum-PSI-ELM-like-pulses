...
from functions.spectools import rotate,do_tilt, binData, get_angle, get_tilt,get_angle_2,get_4_points
...
try:
	...
	overwrite_everything = [False, False, False, False]
...
if (merge_ID_target<=26 or merge_ID_target==53):
 ...
 [1608,    0]])
	...
	binnedSens = np.load('/home/ffederic/work/Collaboratory/test/experimental_data/functions/Calibrations/sensitivity_'+str(type_of_sensitivity)+'/Sensitivity_4.npy')
	# limits_angle = [66,84]
	# limits_tilt = [66,84]
	# limits_wave = [66,84]
	limits_angle = [0,0]
	...
	if (last_merge_done<=100 and last_merge_done>=85):
		no_calculate_presets = 1
...
if no_calculate_presets==0:
	...
	if np.sum(limits_tilt)!=0:
		...
		if np.sum(np.isnan(tilt_4_points))>0:
			tilt_4_points = np.array([[0,np.shape(data)[0]],
...
# if True:
	...
	for j in all_j:
		...
		if isinstance(df_log.loc[j,['current_trace_file']][0],str):
			...
			plt.plot(1000*(time_of_pulses-np.linspace(0,(len(time_of_pulses)-1),(len(time_of_pulses)))*0.1),'c',label='time when the pulse starts (current), mean increment ='+str(np.around( 1000*(time_of_pulses-np.linspace(0,(len(time_of_pulses)-1),(len(time_of_pulses)))*0.1)[-1]/(len(time_of_pulses)-1), decimals=5)))
			...
			plt.plot( 1000*(time_of_beginning_of_frame -np.linspace(0,(len(time_of_pulses)-1),(len(time_of_pulses)))*0.1),'r',label='tine when record starts, mean increment ='+str(np.around( 1000*(time_of_beginning_of_frame -np.linspace(0,(len(time_of_pulses)-1),(len(time_of_pulses)))*0.1)[-1]/(len(time_of_pulses)-1), decimals=5)))
			plt.grid()
			...
			plt.figure(figsize=(20, 10))
			plt.plot(1000*(time_of_pulses-time_of_beginning_of_frame))
			plt.plot(good_pulses-1,1000*(time_of_pulses-time_of_beginning_of_frame)[good_pulses-1],'+')
			plt.grid()
			plt.title('Time shift between camera and pulse current in '+str(j)+'\n mean inctement ='+str(np.around( 1000*(time_of_pulses-time_of_beginning_of_frame)[-1]/(len(time_of_pulses-time_of_beginning_of_frame)-1), decimals=5)))
			plt.xlabel('pulse index')
			plt.ylabel('time shift [ms]')
			# plt.legend(loc='best')
			...
			plt.figure(figsize=(20, 10))
			plt.plot(good_pulses-1,(1000*(time_of_pulses-time_of_beginning_of_frame) - np.linspace(0,(len(time_of_pulses)-1),(len(time_of_pulses)))* incremental_step )[good_pulses-1])
			plt.plot(good_pulses-1,(1000*(time_of_pulses-time_of_beginning_of_frame) - np.linspace(0,(len(time_of_pulses)-1),(len(time_of_pulses)))* incremental_step )[good_pulses-1],'+')
			plt.grid()
	...
	if time_resolution_scan:
		np.savez_compressed(path_where_to_save_everything+'/merge'+str(merge_ID_target)+'_skip_of_'+str(time_resolution_extra_skip+1)+'row_merge_tot',merge_values=merge_values,merge_time=merge_time,merge_row=merge_row,merge_Gain=merge_Gain,merge_overexposed=merge_overexposed)
		...
		np.savez_compressed(path_where_to_save_everything+'/merge' + str(merge_ID_target) + '_merge_tot',merge_values=merge_values,merge_time=merge_time,merge_row=merge_row,merge_Gain=merge_Gain,merge_overexposed=merge_overexposed)
	...
	for j in all_j:
		...
		if not os.path.exists(path_where_to_save_everything):
			os.makedirs(path_where_to_save_everything)
		ani = coleval.movie_from_data(np.array([np.array(data_all)]), 1000/incremental_step, row_shift, 'Wavelength axis [pixles]', 'Row axis [pixles]','Intersity [au]',extvmin=0,extvmax=30)
		# plt.show()
		ani.save(path_where_to_save_everything + '/merge'+str(merge_ID_target)+'_scan_n'+str(j)+'_original_data' + '.mp4', fps=5, extra_args=['-vcodec', 'libx264'])
		plt.close()
...
plt.figure(figsize=(20, 10))
plt.imshow(data_sum,'rainbow',origin='lower')
plt.colorbar()
for i in range(41):
	plt.plot([0,np.shape(data_sum)[-1]],[geom['bin00b']+i*geom['binInterv'],geom['bin00b']+i*geom['binInterv']],'--k')
...
if (((not time_resolution_scan and not os.path.exists(path_where_to_save_everything+'/merge'+str(merge_ID_target)+'_composed_array.npy')) or (time_resolution_scan and not os.path.exists(path_where_to_save_everything+'/merge'+str(merge_ID_target)+'_skip_of_'+str(time_resolution_extra_skip+1)+'row_composed_array.npy'))) or overwrite_everything[1]):
	...
	if False:	# I noticed that the counts are smooth within the same row so I tried to fit only for different times but it is still very irregular between rows
		...
		time_range_for_interp = conventional_time_step
		max_time_expansion = time_range_for_interp
		rows_range_for_interp = 3
		max_row_expansion = 2
		min_points_for_interp = 10
		...
		def calc_stuff(arg,row_steps=row_steps,merge_time=merge_time,merge_row=merge_row,merge_values=merge_values,time_range_for_interp=time_range_for_interp,rows_range_for_interp=rows_range_for_interp,calc_stuff_output=calc_stuff_output):
			i = arg[0]
			...
			for j, interpolated_row in enumerate(row_steps):
				...
				for z, interpolated_wave in enumerate(range(1608)):
					values = merge_values[selected,z]
					fit = least_squares(polynomial_2D_residuals_2([merge_time_selected,merge_row_selected,values,interpolated_time,interpolated_row]),guess)
					# print(fit.x)
					...
					composed_array[j,z] = polynomial_2D_2([interpolated_time,interpolated_row],fit.x)
					guess = fit.x
						...
						if ( np.max(number_of_points_for_interpolation[:j]) == 0 or  np.max(number_of_points_for_interpolation[j+1:]) == 0 ):
							...
							for z, interpolated_wave in enumerate(range(1608)):
								...
								if z == max_value_index:
									...
									plt.figure(figsize=(10, 10))
									plt.tricontourf(xv.flatten(), yv.flatten(), polynomial_2D_2([xv.flatten(), yv.flatten()], fit.x),15,cmap='rainbow')
									plt.plot(merge_time_selected, merge_row_selected, '+k')
									plt.plot(interpolated_time, interpolated_row, '+r', markersize=15)
									plt.colorbar().set_label('counts [au]')
	...
	plt.figure(figsize=(20, 10))
	plt.imshow(interpolation_borders_expanded_row, origin='lower',extent=[0,np.shape(interpolation_borders_expanded_row)[1]-1,np.min(new_timesteps),np.max(new_timesteps)],aspect=100)
	...
	plt.figure(figsize=(20, 10))
	plt.imshow(interpolation_borders_expanded_time, origin='lower',extent=[0,np.shape(interpolation_borders_expanded_time)[1]-1,np.min(new_timesteps),np.max(new_timesteps)],aspect=100)
	...
	plt.figure(figsize=(20, 10))
	plt.imshow(number_of_points_for_interpolation, origin='lower',extent=[0,np.shape(number_of_points_for_interpolation)[1]-1,np.min(new_timesteps),np.max(new_timesteps)],aspect=100)
...
#
	...
	def calc_stuff(arg,composed_array=composed_array,geom=geom,calc_stuff_output=calc_stuff_output):
		import numpy as np
		from functions.spectools import rotate
		from functions.fabio_add import four_point_transform
		from functions.spectools import binData
		index = arg[0]
		time = arg[1]
		...
		output = calc_stuff_output(time,binnedData)
		return output
...
if (((not time_resolution_scan and not os.path.exists(path_where_to_save_everything +'/merge'+str(merge_ID_target)+'_all_fits.npy')) or (time_resolution_scan and not os.path.exists(path_where_to_save_everything +'/merge'+str(merge_ID_target)+'_all_fits.npy'))) or overwrite_everything[3]):
	...
	def calc_stuff(arg,binned_data=binned_data,df_settings=df_settings,df_log=df_log,geom=geom,waveLcoefs=waveLcoefs,binnedSens=binnedSens,calc_stuff_output=calc_stuff_output):
		import numpy as np
		from functions.SpectralFit import doSpecFit_single_frame
		index = arg[0]
		time = arg[1]
		...
		output = calc_stuff_output(time, fit)
		return output
...
profile_centre = doLateralfit_single(df_settings, all_fits_ss, merge_ID_target,dx,xx,r,same_centre_every_line=True)[0]

doLateralfit_time_tependent(df_settings, all_fits, merge_ID_target,new_timesteps,dx,xx,r,same_centre_every_line=False,force_glogal_center=profile_centre)#17.5/1000)
	...
	for time_step in sample_time_step:
		...
		for index, loc in enumerate(r[sample_radious]):
			if (loc > 0 and loc < 0.015):
				to_plot = np.divide(np.pi * 4 * np.mean(inverted_profiles[time_step-1:time_step+2, :, index],axis=0),statistical_weigth * einstein_coeff * energy_difference / J_to_eV)
		...
		for index, loc in enumerate(r[sample_radious]):
			if (loc > 0 and loc < 0.015):
				...
				plt.plot(energy_difference, to_plot / np.min(to_plot[relative_index]),label='axial pos=' + str(np.around(1000*loc, decimals=1)))
		plt.legend(loc='best')
		...
		for index, loc in enumerate(r[sample_radious]):
			if (loc > 0 and loc < 0.015):
				to_plot = np.divide(np.pi * 4 * np.mean(inverted_profiles[time_step-1:time_step+2, :, index],axis=0),statistical_weigth * einstein_coeff * energy_difference / J_to_eV)
				plt.plot(energy_difference, to_plot, label='axial pos=' + str(np.around(1000*loc, decimals=1)))
		plt.legend(loc='best')
		plt.title('Boltzmann plot at ' + str(np.around(new_timesteps[time_step], decimals=2)) + '+/-'+str(conventional_time_step)+' ms')
		plt.semilogy()
		plt.semilogx()
		plt.xlabel('trensition energy [eV]')
		plt.ylabel('population density/statistical weight scaled to the min value [#/m^3]')
		...
		for iR in range(np.shape(inverted_profiles)[1]):
			plt.plot(r * 1000, np.divide(np.pi * 4 * np.mean(inverted_profiles[time_step-1:time_step+2, iR],axis=0), einstein_coeff[iR] * energy_difference[iR] / J_to_eV),color[iR], label='n=' + str(iR + 4))
		plt.legend(loc='best')
...
for iR in range(np.shape(SS_inverted_profiles)[0]):
	plt.plot(r * 1000, np.divide(np.pi * 4 * SS_inverted_profiles[iR], einstein_coeff[iR] * energy_difference[iR] / J_to_eV),color[iR], label='n=' + str(iR + 4))
plt.legend(loc='best')
