...
for merge_ID_target in range(17,31,1):
	try:
		...
		if True:
			...
			for j in all_j:
				...
				bad_pulses_indexes=[]
				if j==264:
					CB_to_OES_initial_delay+=0.1

				for index in range(len(Te_prof_multipulse)):
					if (-CB_to_OES_initial_delay-incremental_step*index <-3):	#I want to exlude some unnecessary steady state
						continue
					if (index==0 or ((index+1) in bad_pulses_indexes) ):
						...
						merge_time.append(-CB_to_OES_initial_delay-incremental_step*index)
						continue
					merge_time.append(-CB_to_OES_initial_delay-incremental_step*index)
		...
		plt.imshow(merge_Te_prof_multipulse, 'rainbow',extent=[-40,+40,max(merge_time),min(merge_time)],aspect=50)
		plt.title('Te for merge ' + str(merge_ID_target))
		plt.xlabel('radial location [mm]')
		plt.ylabel('time [ms]')
		plt.colorbar().set_label('[eV]')
		fig.add_subplot(1, 2, 2)
		plt.imshow(merge_ne_prof_multipulse, 'rainbow',extent=[-40,+40,max(merge_time),min(merge_time)],aspect=50)
		...
		rows = 5
		fig.add_subplot(1, 2, 1)
		plt.imshow(merge_Te_prof_multipulse, 'rainbow',vmax=1,extent=[-40,+40,max(merge_time),min(merge_time)],aspect=50)
		plt.title('Te for merge ' + str(merge_ID_target))
		plt.xlabel('radial location [mm]')
		plt.ylabel('time [ms]')
		plt.colorbar().set_label('[eV]')
		fig.add_subplot(1, 2, 2)
		plt.imshow(merge_ne_prof_multipulse, 'rainbow',vmax=10e20,extent=[-40,+40,max(merge_time),min(merge_time)],aspect=50)
		plt.title('ne for merge ' + str(merge_ID_target))
		plt.xlabel('radial location [mm]')
		plt.ylabel('time [ms]')
		plt.colorbar().set_label(['10^20 #/m^3'])
		# plt.show()
		plt.savefig('/home/ffederic/work/Collaboratory/test/experimental_data/merge' + str(merge_ID_target) + '/TS_values_merge_SS_' + str(merge_ID_target) + '.eps',bbox_inches='tight')
		plt.close()
		...
		rows = 5
		fig.add_subplot(1, 2, 1)
		plt.imshow(merge_dTe_multipulse, 'rainbow',extent=[-40,+40,max(merge_time),min(merge_time)],aspect=50)
		plt.title('dTe for merge ' + str(merge_ID_target))
		plt.xlabel('radial location [mm]')
		plt.ylabel('time [ms]')
		plt.colorbar().set_label('[eV]')
		fig.add_subplot(1, 2, 2)
		plt.imshow(merge_dne_multipulse, 'rainbow',extent=[-40,+40,max(merge_time),min(merge_time)],aspect=50)
		...
		print('merge '+str(merge_ID_target)+' FAILED!')
...
data_all.append(np.zeros_like(dataDark))
	...
	#
		ani = coleval.movie_from_data(np.array([np.array(data_all)]), 1000/incremental_step, row_shift, 'Wavelength axis [pixles]', 'Row axis [pixles]','Intersity [au]',extvmin=0)
		# plt.show()
...
if ((not time_resolution_scan and not os.path.exists('/home/ffederic/work/Collaboratory/test/experimental_data/merge'+str(merge_ID_target)+'_composed_array.npy')) or (time_resolution_scan and not os.path.exists('/home/ffederic/work/Collaboratory/test/experimental_data/merge'+str(merge_ID_target)+'_skip_of_'+str(time_resolution_extra_skip+1)+'row_composed_array.npy'))):
	...
	for i,interpolated_time in enumerate(new_timesteps):
		...
		for j, interpolated_row in enumerate(row_steps):
			...
			# else:
			# 	weight_row = weight
			sample_values_2 = sample_values_1[
				np.abs(sample_row_1 - interpolated_row) < rows_range_for_interp + additional_row_range]
			sample_time_2 = sample_time_1[
				np.abs(sample_row_1 - interpolated_row) < rows_range_for_interp + additional_row_range]
			sample_row_2 = sample_row_1[
				np.abs(sample_row_1 - interpolated_row) < rows_range_for_interp + additional_row_range]
			weight_row = 1 / np.sqrt(
				0.00001 + (np.abs(sample_time_2 - interpolated_time) / row_shift) ** grade_of_interpolation + np.abs(sample_row_2 - interpolated_row) ** grade_of_interpolation)
			# weight_row = row_selection * weight
...
doLateralfit_time_tependent(df_settings, all_fits, merge_ID_target,np.min(new_timesteps),np.max(new_timesteps),conventional_time_step)
...
try:
	for time_step in [10, 15, 18, 19, 20, 25, 30]:
		# time_step=19
		plt.figure(figsize=(20, 10))
		to_plot_all = []
		for index, loc in enumerate(r[::2]):
		...
		for index, loc in enumerate(r[::2]):
			if (loc > 0 and loc < 0.015):
				to_plot = np.divide(np.pi * 4 * inverted_profiles[time_step, :, index],
									statistical_weigth * einstein_coeff * energy_difference / J_to_eV)
				plt.plot(energy_difference, to_plot / np.min(to_plot[relative_index]),
						 label='axial pos=' + str(np.around(loc, decimals=4)))
		plt.legend(loc='best')
		...
		plt.savefig('/home/ffederic/work/Collaboratory/test/experimental_data/merge' + str(
			merge_ID_target) + '/Bplot_relative_time_' + str(np.around(new_timesteps[time_step], decimals=2)) + '.eps',
					bbox_inches='tight')
		plt.close()
...
for index, loc in enumerate([26, 28, 30, 32, 34, 36, 40, 45]):
	...
	for iR in range(np.shape(inverted_profiles)[1]):
		plt.plot(new_timesteps, inverted_profiles[:, iR, loc] / inverted_profiles[reference_point, iR, loc], color[iR],
	...
	plt.legend(loc='best')
	plt.ylim(0, 5)
	plt.savefig('/home/ffederic/work/Collaboratory/test/experimental_data/merge' + str(merge_ID_target) + '/rel_intens_r_' + str(
